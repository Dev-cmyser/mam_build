...
for (const mod of modules) {
	...
	domain: if (fs.existsSync(`${root}/${mod}/CNAME`)) {
		fs.copyFileSync(`${root}/${mod}/CNAME`, `${root}/${mod}/-/CNAME`)
	}

	domain: if (fs.existsSync(`${root}/${mod}/robots.txt`)) {
		fs.copyFileSync(`${root}/${mod}/robots.txt`, `${root}/${mod}/-/robots.txt`)
	}

	jekyll: fs.writeFileSync(`${root}/${mod}/-/.nojekyll`, '')
}
...
function exec(dir, command, ...args) {
	...
	const res = child.spawnSync(app, args, {
